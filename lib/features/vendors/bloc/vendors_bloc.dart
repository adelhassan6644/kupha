...
class VendorsBloc extends HydratedBloc<AppEvent, AppState> {
  final VendorsRepo repo;
  final InternetConnection internetConnection;

  VendorsBloc({required this.internetConnection, required this.repo})
      : super(Start()) {
    ...
    if (await internetConnection.updateConnectivityStatus()) {
      try {
        ...
        response.fold((fail) {
          ...
          VendorsModel? res = VendorsModel.fromJson(success.data);
  ...
  AppState? fromJson(Map<String, dynamic> json) {
    try {
      ...
      if (json['state'] == "Done") {
        return Done(
          list: List<VendorModel>.from(
              jsonDecode(json['list']).map((e) => VendorModel.fromJson(e))),
